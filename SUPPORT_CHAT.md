...
The Support Chat feature provides a real-time communication channel between candidates and the Verboheit Support team. It is designed to be persistent and accessible throughout the candidate's journey, including during active examinations.

## Architecture

The support system follows a clean, modular architecture:

1.  **Component**: `SupportChat.tsx` handles the UI and user interactions.
2.  **Hooks**: Specialized hooks encapsulate state and side effects:
    *   `useListConversations`: Discovers existing support conversations for the user.
    *   `useGetSupportMessages`: Manages the message history and updates for a specific conversation.
    *   `useSendSupportMessage`: Handles the logic for sending new messages.
3.  **Service**: `SupportService.ts` performs the actual API calls via `axios`.
...
*   **Staff (Support) Messages**: Displayed on the left with a white background and border.
...
The support chat is integrated into the `Exam` component via a persistent "Help?" button. This button:
...
2.  **Fetch**: `useGetSupportMessages(id)` -> Load message array into state.
...
To ensure a high-performance, real-time experience comparable to the notification system, we propose the following architecture:

### 1. Centralized State Management (`SupportProvider`)
A new `SupportProvider` context will manage global support state, including active conversations, real-time message updates, and unread counts. This eliminates redundant API calls and state mismatches when navigating between components (e.g., from Dashboard to Exam Portal).

```typescript
interface SupportContextType {
  activeConversation: SupportConversationType | null;
...
The `SupportService` will be enhanced to handle WebSocket connections (similar to `NotificationService`), allowing for instantaneous message delivery without polling.
...
- **Events**:
  ...
  - `presence_update`: Indicates the online/offline status of support staff.

### 3. Unified Hook (`useSupportChat`)
...
| Feature | Notification System | Support Chat (Proposed) |
| :--- | :--- | :--- |
| **Transport** | WebSocket + REST | WebSocket + REST |
| **Provider** | `NotificationProvider` | `SupportProvider` |
