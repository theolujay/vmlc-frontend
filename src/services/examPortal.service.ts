...
import {
  ...
  SessionType,
  TakeExamType,
  UpdatedSessionQuestionType,
} from "@/types/Examtype";
import {
  BulkActionType,
  BulkArchiveType,
  BulkPayloadType,
  CandidateSubmitAnswerType,
} from "@/types/Index";
...
export class ExamPortal {
  static async examInfo(): Promise<DashboardType> {
    try {
      const response = await client.get(
        candidateUrls.candidate_exams_dashboard,
      );
  ...
  static async viewExamQuestions(
    id: string,
    page: number = 1,
    filters: Record<string, string> = {},
  ): Promise<UpdatedSessionQuestionType> {
    try {
      ...
      const response = await client.get(
        `${examUrls.EXAM_DETAILS(id)}?${queryParams.toString()}`,
      );
      return response.data;
    } catch (error) {
      console.error(error);
  ...
  static async updateQuestion(
    id: number,
    payload: CreateQuestionType | FormData,
  ) {
    try {
      const response = await client.patch(
        examUrls.UPDATE_QUESTION(id),
        payload,
        {
          headers:
            payload instanceof FormData
  ...
  static async listQuestions(
    page: number = 1,
    filters: Record<string, string> = {},
  ): Promise<QuestionPoolType | undefined> {
    try {
      const queryParams = new URLSearchParams({
        page: page.toString(),
        ...Object.fromEntries(
      ...
      const response = await client.get(
        examUrls.LIST_QUESTIONS(queryParams.toString()),
      );
      return response.data;
    } catch (error) {
      console.error(error);
  ...
  static async bulkActionQuestions(payload: BulkActionType) {
    try {
      const response = await client.post(
        examUrls.BULK_ACTION_QUESTIONS,
        payload,
      );
      return response.data;
    } catch (error) {
  ...
  static async candidateSubmitAnswers(
    exam_id: string,
    payload: CandidateSubmitAnswerType,
  ) {
    try {
      const response = await client.post(
        examUrls.CANDIDATE_SUBMIT_ANSWERS(exam_id),
        payload,
      );
      return response.data;
    } catch (error) {
  ...
  static async getCandidateExamHistory(candidate_id: string) {
    try {
      const response = await client.get(
        examUrls.CANDIDATE_EXAM_HISTORY(candidate_id),
      );
      return response.data;
    } catch (error) {
      console.error(error);
