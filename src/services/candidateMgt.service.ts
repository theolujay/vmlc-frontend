...
export class CandidateMgtService {
  static async getCandidateList(
    ...
    try {
      ...
      const response = await client.get(
        candidateUrls.LIST_CANDIDATES(queryParams.toString()),
      );
  ...
  static async getLeaderBoard(
    ...
    try {
      ...
      const response = await client.get(
        candidateUrls.get_leaderboard(queryParams.toString()),
      );
      return response.data;
    } catch (error) {
      console.error(error);
  ...
  static async getLeaderBoardCandidateDetail(
    stage: string,
    round: string,
    candidate_id: string,
  ): Promise<ViewCandidateDetailType> {
    try {
      const response = await client.get(
        candidateUrls.GET_LEADERBOARD_CANDIDATE_DETAIL(
          stage,
          round,
          candidate_id,
        ),
