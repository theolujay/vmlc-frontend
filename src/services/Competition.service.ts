...
export class CompetitionService {
  ...
  static async listRankings(page: number = 1) {
    try {
      const response = await client.get(competitionUrls.LIST_RANKINGS, {
        params: { page },
      });
      return response.data;
    } catch (error) {
  ...
  static async getCandidateRankingDetail(
    exam_id: string,
    candidate_id: string,
  ) {
    try {
      const response = await client.get(
        competitionUrls.GET_CANDIDATE_RANKING_DETAIL(exam_id, candidate_id),
      );
      return response.data;
    } catch (error) {
      console.error(
        `Error fetching candidate ranking detail for candidate ${candidate_id} in exam ${exam_id}:`,
        error,
      );
      throw error;
    }
  ...
  static async getCandidateLeagueDetail(candidate_id: string) {
    try {
      const response = await client.get(
        competitionUrls.GET_CANDIDATE_LEAGUE_DETAIL(candidate_id),
      );
      return response.data;
    } catch (error) {
      console.error(
        `Error fetching candidate league detail for candidate ${candidate_id}:`,
        error,
      );
      throw error;
    }
