...
import {
  ...
  DirectAccessLoginResponse,
  LoginRequest,
  LogoutRequest,
  RegAvailableType,
  RegisterRequestValueType,
  RegisterStaffRequestValueType,
  SetNewPasswordType,
  VerifyRequest,
} from "@/types/auth";
...
export class AuthService {
  ...
  static async registerCandidate(
    user: RegisterRequestValueType,
  ): Promise<AuthRegisterResponse> {
    try {
      const response = await client.post(
        authUrls.candidate_registeration,
        user,
      );
  ...
  static async registerStaff(
    user: RegisterStaffRequestValueType,
  ): Promise<AuthRegisterResponse> {
    try {
  ...
  static async directAccessLogin(
    passcode: string,
  ): Promise<DirectAccessLoginResponse> {
    try {
  ...
  static async sendOtpForForgotPassword(payload: VerifyRequest) {
    const response = await client.post(
      authUrls.sendOtpForForgotPassword,
      payload,
    );
    return response.data;
  }
  ...
  static async passwordChangeResendOtp(payload: Omit<VerifyRequest, "otp">) {
    const response = await client.post(
      authUrls.passwordChangeResendOtp,
      payload,
    );
    return response.data;
  }
