...
export default function Exam() {
  ...
  const { isPending, data, isError, error } = useCandidateTakeExam(
    examId,
    examStarted,
  );
  const { data: dashboardData, isPending: dashboardPending } =
    useGetExamPortal();
  ...
  useEffect(() => {
    if (!dashboardPending && dashboardData && !isError) {
      ...
      if (
        ...
        activeExam.attempt?.submitted_at
      ) {
  ...
  const currentStage =
    dashboardData?.enrollment_stage_progress?.current_stage || "SCREENING";
  ...
  const handleSelectOption = (questionId: number, displayedOption: string) => {
    ...
    const originalOption =
      (question?._optionMapping as Record<string, string> | undefined)?.[
        displayedOption
      ] || displayedOption;
  ...
  const uiAnswers = useMemo(() => {
    ...
    Object.entries(answers).forEach(([qId, originalOpt]) => {
      ...
      const question = processedData?.questions.find(
        (q) => q.id === questionId,
      );

      if (question?._optionMapping) {
        const displayedOpt = Object.entries(question._optionMapping).find(
          ([, orig]) => orig === originalOpt,
        )?.[0];
  ...
  async function handleSubmit() {
    try {
      ...
      try {
        ...
        console.error(
          "Final proctoring heartbeat failed, proceeding with submission anyway",
          proctorError,
        );
      }
  ...
  if (isError) {
    ...
    const detail =
      ...
      "You cannot access this examination at this time.";
      ...
      return (
        ...
        >
          ...
          >
            ...
            <p className="text-gray-600 mb-8 leading-relaxed">{detail}</p>
            <button
              onClick={() => router.push("/exam-portal")}
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-6 text-center">
        <div className="bg-white p-10 rounded-[2.5rem] shadow-xl max-w-2xl border border-gray-100">
          ...
          <div className="text-left bg-gray-50 p-6 rounded-2xl border border-gray-100 mb-8">
            <p className="text-xs font-black text-[#3E4095] uppercase tracking-widest mb-4">
              Instructions & Rules:
            </p>
            <ul className="space-y-3">
              {[
                ...
                <li
                  key={i}
                  className="flex gap-3 text-sm text-gray-600 leading-relaxed font-medium"
                >
                  <span className="shrink-0 w-5 h-5 rounded-full bg-[#3E4095]/10 text-[#3E4095] flex items-center justify-center text-[10px] font-bold">
                    {i + 1}
                  </span>
                  {text}
                </li>
            ...
            <div
              className={clsx(
                ...
                isAgreed
                  ...
                  : "border-gray-300 group-hover:border-[#3E4095]",
              )}
            >
