            ...
            <div className="flex bg-[#F7F9FC] rounded-[2rem] z-50 flex-col overflow-hidden border border-gray-100 shadow-2xl max-w-md w-full mx-auto font-sans text-center">
                <div className="header bg-white p-8 border-b border-gray-50 flex flex-col items-center">
                    <div className="w-16 h-16 bg-rose-50 rounded-2xl flex items-center justify-center text-rose-500 mb-4 animate-pulse">
                        <i className="fas fa-layer-group text-2xl relative">
                            <i className="fas fa-trash-alt text-xs absolute -bottom-1 -right-1 bg-white rounded-full p-0.5"></i>
                        </i>
                    </div>
                    <h2 className='text-xl font-bold text-gray-800 tracking-tight uppercase'>Bulk Delete Questions</h2>
                    <p className="text-[11px] text-gray-500 font-medium mt-2">You are about to delete {questions.length} question{questions.length > 1 ? 's' : ''}.</p>
                </div>

                <div className="p-8">
                    <p className='text-sm font-bold text-gray-600 leading-relaxed uppercase tracking-tight'>
                        Are you sure you want to permanently remove these questions from the platform?
                        <span className="block mt-1 text-[10px] text-gray-400 font-medium normal-case italic">They will be unassigned from all current exam sessions and archived.</span>
                    </p>
                    ...
                    <div className="flex gap-4 mt-8 w-full">
                        ...
                        <button
                            ...
                            className={clsx(
                                ...
                                (isPending || questions.length === 0) && "opacity-70 cursor-not-allowed translate-y-0 shadow-none"
                            )}
                        >
                            {isPending ? <Spinner /> : (
                                <>
