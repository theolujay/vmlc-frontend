...
const TEMPLATE_COLUMNS = ["email", "first_name", "last_name", "phone", "state", "role", "occupation"];
const ROLE_OPTIONS = [
...
export default function ImportStaffModal({ open, close }: ImportStaffModalProps) {
  const [step, setStep] = useState<"upload" | "review" | "complete">("upload");
  const [file, setFile] = useState<File | null>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const { importStaff, data, isPending, error, reset } = useBulkStaffImport();
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm">
                    ...
                    <p className="text-amber-600 text-xs mt-2">
                      Valid roles: volunteer, moderator, admin, manager<br />
                      Valid states: lagos, ogun, rivers, abuja
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3">
                <button
                  ...
                  className={clsx(
                    ...
                    "bg-[#3E4095] hover:bg-[#2d2f6e] text-white hover:-translate-y-0.5 hover:shadow-[#3E4095]/30 active:translate-y-0"
                  )}
                >
                  {isPending ? (
                    <>
                ...
                <button
                  ...
                  className="w-full py-3 bg-gray-50 text-gray-400 rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-gray-100 transition-all"
                >
                  Cancel
                </button>
              </div>
                  ...
                  {data.success.length > 0 && (
                    <div>
                      <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">
                        Successfully Imported ({data.success.length})
                      </p>
                      <div className="bg-white border border-gray-100 rounded-xl overflow-hidden max-h-40 overflow-y-auto">
                        {data.success.slice(0, 10).map((item, idx) => (
                          <div key={idx} className="flex items-center justify-between px-4 py-2 border-b border-gray-50 last:border-0 text-xs">
                            <span className="font-medium text-gray-700">{item.email}</span>
                            <span className="text-green-600">Created</span>
                          </div>
                        ))}
                        {data.success.length > 10 && (
                          <div className="px-4 py-2 text-center text-xs text-gray-400">
                            ...and {data.success.length - 10} more
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {data.errors.length > 0 && (
                    <div>
                      ...
                      <div className="bg-red-50 border border-red-100 rounded-xl overflow-hidden max-h-40 overflow-y-auto">
                        {data.errors.slice(0, 10).map((item, idx) => (
