...
import {
  BroadcastAnnouncementIcon,
  EmailChannelIcon,
  PlatformChannelIcon,
  SMSIcon,
} from './AnnouncementIconts';
...
export default function Announcement() {
  ...
  const mediumCounts = data?.results.reduce(
    (acc, item) => {
  ...
  return (
    <div className="flex flex-col gap-1 ">
      ...
      <div className="flex flex-col gap-3 mt-3 w-[96%] mx-auto">
        <AnnouncementCard
          platformChannel={mediumCounts?.platform ?? 0}
          emailChannel={mediumCounts?.email ?? 0}
          total={totalBroadCast ?? 0}
        />
...
function AnnouncementCard({
  ...
  return (
    <ResponsiveContainer className="flex gap-1 flex-col">
    ...
    <ResponsiveContainer className="flex gap-4 py-3 px-0 flex-col mx-auto">
      <div className="flex justify-between px-3">
        <div className="flex gap-1 flex-col">
          ...
          <p>
            This table shows the total announcement history sent from the
            platform
          </p>
        </div>
        <div className="flex items-center justify-between gap-2">
          ...
          <button className="inline-flex items-center h-10 gap-2 border rounded-md px-2 py-1 border-[#E4E7EC] cursor-pointer ">
            <span>
              <SortIcon />
            </span>
          ...
          },
          {
            ...
            render: (_, row) => (
              <div className="flex justify-between items-center gap-1">
                <button
                  className="cursor-pointer font-semibold text-[#6941C6]"
