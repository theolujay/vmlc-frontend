...
export default function Announcement() {
    ...
    const { isPending, data } = useGetBroadcast()
    const { page, setPage } = usePagination()
    console.log(data, 'what is the current broadcast')
...
console.log(mediumCounts,'this is medium count');
                ...
                },
                {
                    key: 'mode', header: "Modes", render: (_, row) => {
                        ...
                        return <div className={clsx("flex items-center gap-1")}>
                            <div className='flex items-start gap-1'>{row?.mediums.map((val: any, index: number) => <span key={`medium-${index}`} className={clsx(getAppropriatePlatformColor(val),'px-2 rounded-full')}>{val}</span>)}</div>
                        </div>
                    }
                },
                {
